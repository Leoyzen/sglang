...
#pragma unroll
  ...
  ///
  /// `v_lo` / `v_hi` are the fp32 boundaries of the threshold bin, i.e. the
  /// `coarse_bin_lower_bound<kHistBits>` pair the collect pass classified with;
  /// the fallback must select inside exactly the set that pass counted. Both
  /// scans therefore test `val >= v_lo && val < v_hi` for membership rather
  /// than excluding with `val < v_lo || val >= v_hi`: the ragged entry point
  /// masks the <= 3 columns ahead of a window with `padding_value()` (NaN),
  /// and NaN compares false against both bounds, so the exclusion form would
  /// admit it while the collect pass's two `>=` tests reject it.
...
#pragma unroll
    for (int round = 0; round < 4; ++round) {
      ...
      const auto [exact_threshold, above, equal] = scratch->match;
...
#pragma unroll
    ...
    if (count_eq <= kMaxNumTie) {
      handle_tie(smem->tie_values, problem, count_gt, count_eq, remain_topk, &smem->tie_handle);
    } else {
      handle_oversized_bin(problem, smem->v_lo, smem->v_hi, count_gt, count_eq, smem->tie_values, &smem->tie_handle);
    }
  }
};
 ...
 public:
  ...
  SGL_DEVICE static void forward(TopKProblem problem, void* _smem) {
    ...
    if (this_rank != 0) {
      ...
      // Publish the *unclamped* count so rank 0 can tell an oversized bin from a
      // clipped buffer; the copy loop below still only stores what fits.
      const auto local_count_eq = smem->count_eq;
      const auto local_stored_count_eq = min(local_count_eq, kMaxNumTie);
      if (tx == 0) {
...
#pragma unroll
      for (uint32_t i = 0; i < kTieItems; ++i) {
        const auto t = tx + i * kBlockSize;
        if (t < local_stored_count_eq && local_start_eq + t < kMaxNumTie) {
...
#pragma unroll
      ...
      if (count_eq <= kMaxNumTie) {
        ...
        // The fallback re-scans the complete row. `problem.in` is already the
        // whole row here: for_each_input<kClusterSize> owns the chunk split via
        // blockIdx.y, so no per-rank offset was applied to it.
        handle_oversized_bin(problem, smem->v_lo, smem->v_hi, count_gt, count_eq, smem->tie_values, &smem->tie_handle);
      }
    }
  }
