    ...
    def _check_hybrid_prefetch_result(
        ...
        * Everything else (SWA / Mamba components, mixed DeepSeekV4 stacks):
        ...
        if pool_transfers and not all_succeeded:
            ...
            # Same dict object the caller unpacks, so deletions below are
            # visible to the commit / staging path.
            comp_xfers = self.ongoing_prefetch[request].comp_xfers
            failed_components = [
                ct
            ...
            # Belief self-heal is scoped to the retained KV prefix (relative
            # to the trimmed chain start when a device-resident head was
            # dropped after the availability query).
            invalidation_hashes = (
                operation.sidecar_hash_values
                if operation.sidecar_hash_values is not None
                else hash_value
