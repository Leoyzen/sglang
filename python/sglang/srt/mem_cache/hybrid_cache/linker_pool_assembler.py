...
def _build_deepseek_v4_device_pool_group(
    ...
    if mappings.c4:
        # Unified-KV keeps the compressed region in unified_kv_pool, so the
        # per-ratio c4 pool is legitimately absent there; only a non-unified
        # layout with C4 layers but no pool is a build error.
        if kvcache.c4_kv_pool is None and not is_unified_kv:
            raise ValueError(
