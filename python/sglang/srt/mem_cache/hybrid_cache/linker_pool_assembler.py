...
def _build_mamba_device_pool_group(
    ...
    if page_size != 1:
        ...
        logger.warning(
            "Mamba direct linker running with tree page_size=%d "
            "(mamba slots stay slot-granular).",
            page_size,
        )
    ...
    draft_pools = tuple(
        getattr(pool, "full_kv_pool", pool) for pool in mtp_draft_device_pools
    )
...
def _build_mamba_swa_device_pool_group(
    ...
    if page_size != 1:
        logger.warning(
            "Mamba(SWA) direct linker running with tree page_size=%d "
            "(mamba slots stay slot-granular).",
            page_size,
        )
