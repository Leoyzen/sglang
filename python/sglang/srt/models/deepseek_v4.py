...
def _apply_wo_a_bf16_matmul(
    ...
    Single-token decode uses a GEMV for the validated TP4/TP8 shapes on
    Blackwell or SM90. Blackwell verify batches up to 384 rows and large
    prefill batches write token-major output directly to avoid the layout
    copy before wo_b. ROCm decode can use aiter batched GEMM; other cases
    use torch.einsum.
    """
    ...
    def _run_moe_ffn_dp_sync(
        ...
        *,
        ...
        if _use_tp_attn_a2a_scatter:
            ...
            # The DSpark draft path passes input_ids=None (its MoE gate is not
            # hash-routed and forward_deepep only consumes input_ids_global for
            # is_hash), so keep the token scatters optional like the kwargs.
