...
def _apply_attention_output_gate(module, attn_output, gate):
    ...
    raise RuntimeError(
        "Prepared MLA attention gates are unsigmoided and require a model-specific application hook"
    )
...
class DeepseekMLAForwardMixin:
    ...
    def _can_fuse_bmm_into_attention(
        ...
        if not self.use_dsa:
            return False
        # DCP: the DSA-extend q all-gather path (all_gather_q_for_mla_decode in
        # forward_absorb_prepare) consumes the un-transposed [H, B, D] bmm output
        # written straight into the DCP combine buffer (use_fused_dcp_q_buf /
        # defer_q_nope_transpose). This fusion instead materializes q_nope_out in
        # [B, H, D] layout, which the gather cannot consume — and both features
        # key off the same BCG extend surface, so without this carve-out they
        # co-fire and crash with a cat dim mismatch. The fused buf path keeps the
        # gather's zero-copy fast path under DCP.
    ...
    def forward_absorb_core(
        ...
        if get_parallel().dcp_enabled and (
            ...
            assert lse is not None, (
                "DCP LSE combine reached without an LSE — the attention call above must go through the attn_mqa_for_dcp_decode branch."
            )
