...
class Glm5NextLinearAttention(nn.Module):
    ...
    def _can_fuse_proj(
        ...
        *fused_projs: str,
        ...
        # A global quant_config does not mean these projections are
        # quantized: GLM-5.3-Flash ships FP8 experts with BF16 attention.
        # The fused modules can only eat unquantized weights, so probe each
        # source prefix. Fork-local hardening over the inline isinstance
        # probe: the shared helper requires an explicit
        # UnquantizedLinearMethod and treats a config that raises on the
        # weightless probe (e.g. compressed-tensors) as "not provably
        # unquantized" instead of failing model construction.
        source_projs = [
            proj
        ...
        return are_linear_prefixes_unquantized(
            quant_config, [f"{prefix}.{proj}" for proj in source_projs]
        )
...
class Glm5NextForConditionalGeneration(nn.Module):
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]], is_nextn=False):
        ...
        if not getattr(self, "encoder_only", False):
            layers = [self.model.decoder] if is_nextn else self.model.layers
            for layer in layers:
