...
class C4IndexerBackendMixin:
    ...
    def forward_c4_indexer(
        ...
        if skip_topk and prev_topk_indices is None:
            # Ad-hoc fix: gracefully fall back to full indexer when producer
            # didn't provide raw top-k (e.g. target_verify where raw_indices
            # allocation is skipped to avoid CUDA graph memory pool perturbation).
            skip_topk = False
        ...
        if core_metadata.c4_sparse_raw_indices is not None:
            ...
            # Ad-hoc fix: skip raw_indices allocation for return_topk_indices in
            # target_verify to avoid perturbing CUDA graph memory pool layout.
            # Consumer layers fall back to full indexer (same as index_topk_freq=1).
            raw_indices = torch.empty_like(c4_sparse_page_indices)
