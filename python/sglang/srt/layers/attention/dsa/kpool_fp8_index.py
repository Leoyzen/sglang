...
def _kbuf_page_tokens(pool, buf) -> int:
    """Row width, in tokens, of the index-K buffer being addressed.
...
def gather_index_k_scale_prefix_into(
    ...
    assert pool.page_size % BLOCK_SIZE_K == 0
    ...
    _gather_index_k_scale_prefix_into_kernel[(seq_len,)](
        ...
        k_out,
        scale_out,
        PAGE_SIZE=_kbuf_page_tokens(pool, buf),
        BUF_NUMEL_PER_PAGE=buf.shape[1],
        HEAD_DIM=INDEX_HEAD_DIM,
        S_OFFSET_NBYTES_IN_PAGE=_kbuf_page_tokens(pool, buf) * INDEX_HEAD_DIM,
