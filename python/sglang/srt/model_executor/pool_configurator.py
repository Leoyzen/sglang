...
def _get_dsa_indexer_cache_token_multiplier(kvc: KVCacheConfigurator) -> int:
    ...
    # Main sources enable_hisparse from the resolved memory config
    # (get_memory()), while tests may only populate kvc.server_args; read both.
    enable_hisparse = getattr(kvc.server_args, "enable_hisparse", None)
    if enable_hisparse is None:
        enable_hisparse = get_memory().enable_hisparse
    if enable_hisparse:
        from sglang.srt.mem_cache.sparsity import parse_hisparse_config

        return parse_hisparse_config().host_to_device_ratio
...
def _get_dsa_cache_layer_ids(kvc: KVCacheConfigurator, num_layers: int) -> list[int]:
    ...
    if kvc.mambaish_config and not kvc.is_draft_worker:
        layer_ids = [layer_id for layer_id in kvc.mambaish_config.full_attention_layer_ids if kvc.layer_info.start_layer <= layer_id < kvc.layer_info.end_layer]
    else:
...
class MemoryPoolConfigurator:
    ...
    def calculate_pool_sizes_from_max_tokens(self, max_total_num_tokens: int, page_size: int) -> MemoryPoolConfig:
        """Constraint path: recalculate pool sizes from a constrained max_tokens."""
        raise NotImplementedError

    def finalize_with_max_running_requests(self, config: MemoryPoolConfig) -> MemoryPoolConfig:
        return config

    @staticmethod
    def validate_swa_pool_size(swa_tokens: int, sliding_window_size: Optional[int], page_size: int) -> None:
...
class DefaultPoolConfigurator(MemoryPoolConfigurator):
    ...
    def __init__(self, kvc: KVCacheConfigurator):
        ...
        if mambaish := mambaish_config(kvc.model_config):
            effective_layer_ids = [i for i in mambaish.full_attention_layer_ids if kvc.layer_info.start_layer <= i < kvc.layer_info.end_layer]
        ...
        has_kv_on_another_pp_stage = self._cell_size == 0 and mambaish is not None and bool(mambaish.full_attention_layer_ids) and kvc.ps.pp_size > 1
        self._zero_kv_max_tokens = torch.iinfo(torch.int64).max if has_kv_on_another_pp_stage else get_schedule().max_total_tokens or kvc.model_config.context_len
        ...
        if kvc.spec_algorithm.is_dflash_family() and not kvc.is_draft_worker:
            ...
            if draft_num_layers is not None and int(draft_num_layers) > 0 and int(num_layers) > 0:
    ...
    def _compute_cell_size(self, kvc: KVCacheConfigurator, num_layers: int) -> int:
        ...
        effective_num_layers = num_layers if kvc.server_args.enable_hisparse else get_glm_dsa_layer_split_effective_num_layers(kvc, num_layers)
        ...
        if kvc.use_mla_backend:
            ...
            if is_float4_e2m1fn_x2(kv_cache_dtype):
                # kv_scale_buffer
                scale_block_size = 16
                cell_size = (cell_size // 2) + (((model_config.kv_lora_rank + model_config.qk_rope_head_dim) // scale_block_size) * effective_num_layers * kv_size)
            ...
            indexer_k_only_layer_ids = set(get_minimax_sparse_disable_value_layer_ids(sparse_cfg))

            local_dense_layer_ids = [l for l in dense_layer_ids if kvc.layer_info.start_layer <= l < kvc.layer_info.end_layer]
            local_sparse_layer_ids = [l for l in sparse_layer_ids if kvc.layer_info.start_layer <= l < kvc.layer_info.end_layer]
            num_dense = len(local_dense_layer_ids)
            num_sparse = len(local_sparse_layer_ids)
            num_indexer_k_only = sum(1 for l in local_sparse_layer_ids if l in indexer_k_only_layer_ids)
            ...
            main_pool_bytes = (num_dense + num_sparse) * 2 * kv_heads * head_dim * kv_size
            indexer_bytes = (num_indexer_kv * 2 + num_indexer_k_only) * indexer_head_dim * indexer_dtype_size
            ...
            cell_size = n * (model_config.head_dim + model_config.v_head_dim) * effective_num_layers * kv_size

            if is_float4_e2m1fn_x2(kv_cache_dtype):
                # kv_scale_buffer
                scale_block_size = 16
                k = model_config.head_dim
                cell_size = (cell_size // 2) + ((n * k * effective_num_layers * 2 * kv_size) // scale_block_size)
                # FP4 prefill uses one shared FP8 dequant workspace across layers.
                cell_size += n * k * 2 * kv_size
            elif self.kv_cache_dtype_str == "mxfp8":
                scale_block_size = 32
                cell_size += (n * (model_config.head_dim + model_config.v_head_dim) * num_layers) // scale_block_size
    ...
    def _compute_dsa_indexer_cell_size(
        ...
        *,
        ...
        indexer_size_per_token = index_head_dim + index_head_dim // DSATokenToKVPool.quant_block_size * 4
        element_size = torch._utils._element_size(DSATokenToKVPool.index_k_with_scale_buffer_dtype)
        indexer_multiplier = _get_dsa_indexer_cache_token_multiplier(kvc)

        from sglang.srt.mem_cache.kv_cache_configurator import (
            _should_elide_dsa_index_k,
        )

        if allocate_all_layers or kvc.server_args.enable_hisparse or not _should_elide_dsa_index_k(is_draft_worker=kvc.is_draft_worker):
            ...
            if shard_size > 1:
                ...
                active_indexer_layers = [
                    layer_id for layer_id in range(kvc.layer_info.start_layer, kvc.layer_info.end_layer) if not dsa_layer_skips_topk(kvc.model_config.hf_config, layer_id)
                ]
                ...
                for rank in range(shard_size):
                    ...
                    max_owned = max(
                        max_owned,
                        sum(kvc.layer_info.start_layer + i in active_set for i in range(start, end)),
                    )
                num_indexer_layers = max_owned + 1
            else:
                num_indexer_layers = sum(not dsa_layer_skips_topk(kvc.model_config.hf_config, layer_id) for layer_id in _get_dsa_cache_layer_ids(kvc, num_layers))

        return int(indexer_size_per_token * num_indexer_layers * element_size * indexer_multiplier)

    def calculate_pool_sizes(self, available_bytes: int, page_size: int) -> MemoryPoolConfig:
        max_total_num_tokens = available_bytes // self._cell_size if self._cell_size else self._zero_kv_max_tokens
...
class HybridSWAPoolConfigurator(MemoryPoolConfigurator):
    ...
    def __init__(self, kvc: KVCacheConfigurator):
        ...
        assert self._swa_layers_num > 0, "Hybrid SWA model must have at least one SWA layer"
        ...
        if model_config.attention_arch == AttentionArch.MLA:
            ...
            if is_deepseek_dsa(model_config.hf_config):
                ...
                index_elements = index_head_dim + index_head_dim // DSATokenToKVPool.quant_block_size * 4
                self._full_per_token += index_elements * torch._utils._element_size(DSATokenToKVPool.index_k_with_scale_buffer_dtype)
            self._swa_per_token = (model_config.swa_kv_lora_rank + model_config.swa_qk_rope_head_dim) * kv_size
        else:
            # Full layer per-token memory (bytes)
            self._full_per_token = model_config.get_num_kv_heads(tp_size) * (model_config.head_dim + model_config.v_head_dim) * kv_size

            # SWA layer per-token memory (bytes)
            self._swa_per_token = model_config.get_swa_num_kv_heads(tp_size) * (model_config.swa_head_dim + model_config.swa_v_head_dim) * kv_size

        if self.kv_cache_dtype_str == "mxfp8":
            scale_block_size = 32
            self._full_per_token += (model_config.get_num_kv_heads(tp_size) * (model_config.head_dim + model_config.v_head_dim)) // scale_block_size
            self._swa_per_token += (model_config.get_swa_num_kv_heads(tp_size) * (model_config.swa_head_dim + model_config.swa_v_head_dim)) // scale_block_size
        ...
        if (kvc.spec_algorithm.is_eagle() or kvc.spec_algorithm.is_standalone()) and not kvc.is_draft_worker:
            ...
            if draft_layers is not None and int(draft_layers) > 0:
                ...
                if mtp_local_layer_ids is not None:
                    ...
                    self._draft_swa_full_layers_num = sum(layer_id in local_layer_ids for layer_id in range(draft_layers))
                else:
    ...
    def _recompute_cell_size(self) -> None:
        ...
        if self._full_layers_num == 0:
            ...
            self._cell_size = (
                ...
                + self._swa_per_token * self._draft_swa_full_layers_num
                + self._swa_full_tokens_ratio * self._swa_per_token * (self._swa_layers_num + self._draft_swa_layers_num)
                + self._draft_cell_size
            )

    def _draft_pool_bytes_per_token(self) -> int:
        return int(
            self._full_per_token * self._draft_full_layers_num + self._swa_per_token * (self._draft_swa_layers_num + self._draft_swa_full_layers_num) + self._draft_cell_size
        )
    ...
    def _max_unified_full_tokens(
        ...
        def allocation_bytes(full_pages: int) -> int:
            ...
            swa_tokens = fixed_swa_tokens if fixed_swa_tokens is not None else int(full_tokens * self._swa_full_tokens_ratio) // page_size * page_size
            target_bytes = full_tokens * target_full_bytes_per_token + swa_tokens * target_swa_bytes_per_token
        ...
        full_pages = bisect_right(range(max_pages + 1), available_bytes, key=allocation_bytes) - 1
        return max(full_pages, 0) * page_size

    def _solve_pool_sizes(self, max_total_num_tokens: int, page_size: int) -> MemoryPoolConfig:
        ...
        if self._full_layers_num == 0:
            ...
            logger.info(f"Use sliding window memory pool (all SWA). swa_layer_tokens={swa_tokens}")
        ...
        self.validate_swa_pool_size(swa_tokens, self._sliding_window_size, self._page_size)

        logger.info(f"Use sliding window memory pool. full_layer_tokens={full_tokens}, swa_layer_tokens={swa_tokens}")
    ...
    def calculate_pool_sizes(self, available_bytes: int, page_size: int) -> MemoryPoolConfig:
        if self._enable_unified_memory and self._full_layers_num > 0 and self._draft_pool_bytes_per_token() > 0:
            max_total_num_tokens = self._max_unified_full_tokens(available_bytes, page_size)
        else:
...
class SWAChunkCapPoolConfigurator(HybridSWAPoolConfigurator):
    ...
    def __init__(self, kvc: KVCacheConfigurator):
        ...
        if get_disagg().disaggregation_mode == "decode":
            self._swa_cap = per_request * num_reqs + (window + page_size) * get_disagg().disaggregation_decode_extra_slots
        else:
            chunks_in_flight = 1 if get_schedule().disable_overlap_schedule else 2
            self._swa_cap = per_request * num_reqs + chunks_in_flight * get_schedule().chunked_prefill_size + page_size
    ...
    def calculate_pool_sizes(self, available_bytes: int, page_size: int) -> MemoryPoolConfig:
        ...
        fixed_swa_bytes = swa_tokens * self._swa_per_token * (self._swa_layers_num + self._draft_swa_layers_num)
        if self._enable_unified_memory and self._draft_pool_bytes_per_token() > 0:
            full_tokens = self._max_unified_full_tokens(available_bytes, page_size, fixed_swa_tokens=swa_tokens)
        else:
            full_cell_size = self._full_per_token * (self._full_layers_num + self._draft_full_layers_num) + self._swa_per_token * self._draft_swa_full_layers_num
            full_tokens = (int((available_bytes - fixed_swa_bytes) // full_cell_size) // page_size) * page_size
        if full_tokens <= 0:
            raise RuntimeError(
...
class DSV4PoolConfigurator(MemoryPoolConfigurator):
    ...
    def __init__(self, kvc: KVCacheConfigurator):
        ...
        self.compression_ratios = cfg.compress_ratios[kvc.layer_info.start_layer : kvc.layer_info.end_layer]
        if kvc.ps.pp_size > 1:
            logger.info(
        ...
        self.requested_max_running_requests_per_worker = get_schedule().max_running_requests // kvc.ps.attn_dp_size if get_schedule().max_running_requests is not None else None
        self.disaggregation_mode = get_disagg().disaggregation_mode
        self.disaggregation_decode_extra_slots = get_disagg().disaggregation_decode_extra_slots or 0
        ...
        if envs.SGLANG_OPT_USE_ONLINE_COMPRESS.get():
            allow_experimental_online_c128_mtp = envs.SGLANG_EXPERIMENTAL_ONLINE_C128_MTP.get() and kvc.spec_algorithm.is_eagle()
            ...
            if allow_experimental_online_c128_mtp:
                assert self.online_c128_mtp_max_draft_tokens > 0, "SGLANG_EXPERIMENTAL_ONLINE_C128_MTP requires speculative_num_draft_tokens to be set."
                logger.warning(
    ...
    def _get_bytes_per_full_token(self) -> float:
        ...
        return (
            ...
            + self.swa_ratio * c4_state_ratio * c4_indexer_state_bytes * self.num_layers_ca4
        )
    ...
    def _get_c128_state_fixed_bytes(self, max_running_requests: int) -> int:
        ...
        return state_rows * state_last_dim * c128_state_dtype_size * self.num_layers_ca128

    def _get_c128_state_fixed_bytes_for_token_capacity(self, token_capacity: int) -> int:
        if self.requested_max_running_requests_per_worker is not None:
            return self._get_c128_state_fixed_bytes(self.requested_max_running_requests_per_worker)
    ...
    def calculate_pool_sizes(self, available_bytes: int, page_size: int) -> MemoryPoolConfig:
        ...
        if self.requested_max_running_requests_per_worker is not None:
            c128_state_fixed_bytes = self._get_c128_state_fixed_bytes(self.requested_max_running_requests_per_worker)
        else:
            full_token = int(available_bytes / self.bytes_per_full_token)
            c128_state_fixed_bytes = self._get_c128_state_fixed_bytes_for_token_capacity(full_token)
