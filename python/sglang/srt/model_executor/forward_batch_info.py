...
class ForwardBatch(ForwardBatchDeepSeekMHAMixin):
    ...
    def prepare_mlp_sync_batch(self, model_runner: ModelRunner):
        ...
        # a physical batch whose token count is not a multiple of the per-
        # request verify width misinterprets TP-sync-padded tail rows as
        # broken request groups (logical batch N vs physical padded N+k
        # rows), producing silent wrong results under MTP + TP>1. Align the
        # globally agreed token counts before selecting MAX_LEN/SUM_LEN so
        # active and synchronized idle ranks retain the same collective
        # domain.
