...
def _flashmla_sparse_decode(
    ...
    kv_padded = torch.zeros(padded_tokens, HEAD_DIM, dtype=torch.bfloat16, device=q.device)
...
def main() -> None:
    ...
    if rank == 0:
        ...
        topk_indices = torch.randperm(SEQ_LEN, dtype=torch.int32, device=device, generator=generator)[:TOPK]
    else:
    ...
    local_indices = torch.where(owned, topk_indices // world_size, torch.full_like(topk_indices, -1))
    ...
    if rank == 0:
        reference_out, reference_natural_lse = _flashmla_sparse_decode(q, global_kv, topk_indices)
        ...
        max_lse_error = (global_lse - reference_natural_lse.float() * LOG2_E).abs().max().item()
        print(f"FlashMLA KV DCP SM90 passed: max_out_error={max_out_error:.6f}, max_lse_error={max_lse_error:.6f}")
