...
def _make_dcp_page_tables(batch_size: int, seq_len: int, dcp_size: int, device: str) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    for batch_id in range(batch_size):
        generator = torch.Generator(device=device).manual_seed(1009 * dcp_size + batch_id)
        # Give each request a disjoint, permuted global-slot region.
        superpages = torch.randperm(num_superpages, generator=generator, device=device) + batch_id * num_superpages
...
def main() -> None:
    ...
    for dcp_size in (2, 4, 8):
        for batch_size, seq_len in ((4, 4096), (4, 16385), (2, 65537)):
            width = (seq_len + 3) & ~3
            scores = torch.randn(batch_size, width, dtype=torch.float32, device=device)[:, :seq_len]
            seq_lens = torch.full((batch_size,), seq_len, dtype=torch.int32, device=device)
            page_table_1, real_page_table = _make_dcp_page_tables(batch_size, seq_len, dcp_size, device)

            raw = torch.topk(scores, TOPK, dim=-1, sorted=False).indices
            unfused_global = torch.gather(page_table_1, 1, raw).to(torch.int32)

            fused_global = torch.full((batch_size, TOPK), -1, dtype=torch.int32, device=device)
            ...
            for dcp_rank in range(dcp_size):
                ...
                for row in range(batch_size):
                    ...
                    unfused_set = set(unfused_local[row][unfused_local[row] >= 0].tolist())
