...
Port of #36904 (beastllama) re-based onto glm-dcp-0907; requires SM89+
...
tilelang_kernel = pytest.importorskip("sglang.kernels.ops.attention.dsa.tilelang_kernel")
...
def _make_inputs(seed):
    ...
    idx = torch.randint(1, POOL, (S, 1, TOPK), device="cuda", generator=g).to(torch.int32)
...
def test_fp8_spread_within_budget_and_negative_control_fails():
    ...
    assert rel_bad > 0.04, f"scrambled indices still matched the reference — the comparator is not discriminating (rel err {rel_bad:.5f})"
