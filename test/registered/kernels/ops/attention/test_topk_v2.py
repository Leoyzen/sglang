...
grid below brackets the fixed boundaries (8192/8193, 16384/16385, 32768/32769)
and the cluster batch cap (512/513) exactly, and spans the arch-dependent ones,
across k in {512,1024,2048} and identity/perm page tables.

An oversized coarse threshold bin (> kMaxNumTie candidates) must be re-scanned
exactly rather than clipped to the tie buffer; the ``oversized_*`` tests below
cover every dispatch path for that fallback.
"""
...
FIXED_CONFIGS = [
    ...
    # --- long rows, large batch (> 512 => non-cluster streaming) ---
    (513, 65537),
]
