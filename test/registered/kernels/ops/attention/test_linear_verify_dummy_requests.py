...
CUDA-gated kernel tests (port of #37587's dummy-request contract): TP-sync
pads an eager verify's request domain, and the fabricated rows carry no state
(slot -1). The kernels must (a) never read the padded rows' inputs, gates, or
parent links, and (b) initialize the physical output rows those programs own,
because downstream layers consume them.
"""
...
if not torch.cuda.is_available():
    pytest.skip(
        "Dummy-request kernel contract needs CUDA (triton kernels).",
        allow_module_level=True,
    )

from sglang.kernels.ops.attention.fla.fused_kda_conv_recurrent_verify import (  # noqa: E402
    fused_kda_conv_gating_verify,
)
from sglang.kernels.ops.attention.fla.fused_sigmoid_gating_recurrent import (  # noqa: E402
    fused_sigmoid_gating_delta_rule_update,
)
from sglang.kernels.ops.mamba.causal_conv1d_triton import (  # noqa: E402
...
def test_conv_dummy_output_is_initialized(steps, dtype, tree):
    ...
    x = torch.randn(2, steps, 65, device=_DEVICE, dtype=dtype).transpose(1, 2)
    x[1].fill_(float("nan"))
    state = torch.randn(4, 3, 65, device=_DEVICE, dtype=dtype).transpose(1, 2)
    before = state.clone()
    weight = torch.randn(65, 4, device=_DEVICE, dtype=dtype)
    indices = torch.tensor([1, -1], device=_DEVICE, dtype=torch.int32)
    intermediate_indices = torch.tensor([0], device=_DEVICE, dtype=torch.int32)
    window = torch.full((2, steps, 3, 65), 73.0, device=_DEVICE, dtype=dtype).transpose(
        2, 3
    )
    ...
    if tree:
        tree_kwargs = {
            "retrieve_next_token": torch.tensor(
                [list(range(1, steps)) + [-1]], device=_DEVICE, dtype=torch.int32
            ),
            "retrieve_next_sibling": torch.full(
                (1, steps), -1, device=_DEVICE, dtype=torch.int32
            ),
            "retrieve_parent_token": torch.empty(
                (1, steps), device=_DEVICE, dtype=torch.int32
            ),
        }
...
def test_recurrent_dummy_skips_inputs_and_short_metadata(is_kda, tree, real_requests):
    ...
    kwargs = {"device": _DEVICE, "dtype": torch.bfloat16}
    ...
    state = torch.randn(7, heads, dim, dim, device=_DEVICE) * 0.2
    before = state.clone()
    ...
    intermediate_indices = torch.arange(
        real_requests, device=_DEVICE, dtype=torch.int32
    )
    parents = (
        torch.tensor(
            [[-1, 0, 0, 1, 1, 2]] * real_requests, device=_DEVICE, dtype=torch.int32
        ).reshape(real_requests, steps)
        if tree
        else None
    )
    scratch = torch.full((5, steps, heads, dim, dim), 73.0, device=_DEVICE)
    reference_scratch = scratch.clone()
    a_log = torch.randn(heads, device=_DEVICE) * 0.2
    dt_bias = torch.randn(a.shape[-1], device=_DEVICE) * 0.2
    ...
    def run(count, cache):
        ...
        return fused_sigmoid_gating_delta_rule_update(
            ...
            cu_seqlens=torch.arange(0, n + 1, steps, device=_DEVICE, dtype=torch.int32),
            is_kda=is_kda,
...
def test_fused_kda_dummy_output_and_state():
    ...
    state = torch.randn(5, heads, dim, dim, device=_DEVICE) * 0.2
    before = state.clone()
    window = torch.full((5, steps, 3, channels), 73.0, **kwargs)
    scratch = torch.full((5, steps, heads, dim, dim), 73.0, device=_DEVICE)
    indices = torch.tensor([2, -1, -1, -1], device=_DEVICE, dtype=torch.int32)
    intermediate_indices = torch.tensor([0], device=_DEVICE, dtype=torch.int32)
    weight = torch.randn(channels, 4, **kwargs) * 0.2
    a_log = torch.randn(heads, device=_DEVICE) * 0.2
    dt_bias = torch.randn(heads * dim, device=_DEVICE) * 0.2
