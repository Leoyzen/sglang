...
class TestDSAFlashMLAKVDCP(unittest.TestCase):
    ...
    def test_returns_base2_lse_and_zero_row_metadata(self, mock_flashmla, mock_fixup):
        ...
        backend.get_device_int32_arange = lambda length: torch.arange(length, dtype=torch.int32)
        ...
        torch.testing.assert_close(fixup_args[2], torch.tensor([2, 0], dtype=torch.int32))
        torch.testing.assert_close(fixup_args[3], torch.tensor([0, 1, 2], dtype=torch.int32))
