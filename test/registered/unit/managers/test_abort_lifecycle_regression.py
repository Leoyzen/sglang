...
def _make_tm(case, tokenizer_worker_num: int = 1) -> TokenizerManager:
    ...
    # dsv4-h200-0820 branch predates the config-bags refactor (#35906): abort_request
    # reads tokenizer_worker_num from self.server_args, not get_serving(). Set it
    # explicitly so the rid_to_state guard is exercised (MagicMock() == 1 is False).
...
def _aborts_sent(tm) -> list:
    ...
    return [
        rid for _, (obj_name, rid) in tm.log.kinds("sent") if obj_name == "AbortReq"
    ]
...
class TestDisconnectCleanupOrdering(CustomTestCase):
    def test_cancelled_error_dispatches_abort_before_pop(self):
        ...
        pop_events = [i for i, e in enumerate(tm.log.events) if e == ("pop", rid)]
        self.assertEqual(
            len(abort_events),
            1,
