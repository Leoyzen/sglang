...
# The tilelang-backed branch is patched out below, but patch() must still
# resolve the target module. Stub the tilelang dependency so the CPU test
# host (without tilelang installed) can import it.
_FAKE_TILELANG_MODULES = {}
for _name in (
    "tilelang",
    "tilelang.language",
    "tilelang.jit",
    ...
    _FAKE_TILELANG_MODULES[_name] = MagicMock()
...
class TestKPoolMqaBackend(CustomTestCase):
    def _run_paged_mqa_dispatch(self, forward_mode, metadata_rows=None):
        ...
        with (
            patch.dict("sys.modules", _FAKE_TILELANG_MODULES),
    ...
    def test_target_verify_cache_write_passes_ragged_layout_through(self):
        # HEAD supports compact ragged verify: the fixed-width draft-group trim
        # must not fire, and the ragged KPool plan flows through untouched.
        indexer = object.__new__(dsa_indexer_kpool.IndexerKPool)
        indexer.index_kpool_compress_ape = torch.empty(4, 128)
        indexer.scale_fmt = None
        indexer._get_q_k_bf16 = MagicMock(
        ...
        plan = SimpleNamespace(
            num_draft_tokens=3,
            req=torch.tensor([17, 18]),
            write_start=torch.tensor([23, 24], dtype=torch.int32),
            tail_logical_start=torch.tensor([19, 20], dtype=torch.int32),
            write_loc=torch.tensor([[29], [30]]),
        ...
        kwargs = write.call_args.kwargs
        self.assertEqual(kwargs["key"].shape, (4, 128))
        self.assertEqual(kwargs["req_pool_indices"].shape, (2,))
        torch.testing.assert_close(kwargs["req_pool_indices"], torch.tensor([17, 18]))
        torch.testing.assert_close(kwargs["out_cache_loc"], torch.arange(4))
    ...
    def test_write_contract_rejects_mismatched_request_domain(self):
        pool = SimpleNamespace(index_kpool=4, tail_extra_slots=1)
        key = torch.randn(12, 128, dtype=torch.bfloat16)
        score = torch.randn(12, 128)
        tail = torch.empty(1, 5, 128, dtype=torch.bfloat16)
        tail_score = torch.empty(1, 5, 128)
        with self.assertRaises(AssertionError):
            kpool_fp8_index.kpool_write_tail_and_maybe_compress(
                ...
                req_pool_indices=torch.tensor([17]),
                write_start=torch.tensor([23] * 3, dtype=torch.int32),
                tail_logical_start=torch.tensor([19] * 3, dtype=torch.int32),
                write_loc=torch.zeros(3, 2, dtype=torch.int32),
