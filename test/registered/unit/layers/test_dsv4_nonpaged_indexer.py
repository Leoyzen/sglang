...
class TestDSV4TopKDispatch(CustomTestCase):
    def test_v2_raw_output_uses_sparse_prefill_buffer_with_capture(self):
        ...
        # The branch's chunk guard reads this in `_is_capture` (indexer.py)
        # before the oversize-varlen path is ruled out by fp8-paged-mqa-logits.
        ...
        with (
            ...
            # _is_cp reads get_parallel().attn_cp_size outside the oversize
            # path's short-circuit guards; the CP group is uninitialized in
            # unit tests, so stub it at the default (cp=1) instead.
            patch(
                f"{_INDEXER}.get_parallel",
...
class TestDSV4TopkV2MetadataRegeneration(CustomTestCase):
    ...
    def test_v2_metadata_regenerates_for_chunked_path(self):
        ...
        def fake_v2(scores, seq_lens, pt, out, ps, metadata, raw_output, **kw):
