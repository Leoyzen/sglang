...
class TestFlashInferTrtllmMoeAllReduce(CustomTestCase):
    ...
    def test_happy_path_defers_to_next_layer_without_duplicate_fusion(self):
        ...
        # The local tree's mHC fused-finalize gate is read unconditionally.
        object.__setattr__(moe, "_fuse_finalize_all_reduce", False)
        object.__setattr__(
            moe, "_maybe_quant_moe_input_once", MagicMock(return_value=None)
        )
        gate = MagicMock(return_value=torch.zeros(1, 8, device=device))
        # The local VL-routing branch diverts on a non-None bias, which a bare
        # MagicMock would report; the plain top-k is what this path exercises.
        gate.e_score_correction_bias_vl = None
        object.__setattr__(moe, "gate", gate)
        object.__setattr__(
            moe,
            "topk",
        ...
        with (
            ...
            payload, unchanged_residual, _ = producer.forward(
                ...
                # The local MoE forward reads this unconditionally.
