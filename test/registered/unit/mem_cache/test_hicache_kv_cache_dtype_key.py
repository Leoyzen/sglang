...
class TestHiCacheDtypeKeyCollision:
    ...
    def test_different_dtype_produces_different_suffix(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert cache_bf16.config_suffix != cache_fp8.config_suffix, f"Suffix collision: bf16={cache_bf16.config_suffix}, fp8={cache_fp8.config_suffix}"
    ...
    def test_none_dtype_backward_compatible(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert "dtype" not in cache.config_suffix, f"dtype leaked into suffix when kv_cache_dtype is None: {cache.config_suffix}"
            assert cache.config_suffix == "_DeepSeek-V4-Flash", f"Unexpected suffix for None dtype: {cache.config_suffix}"
    ...
    def test_non_mla_model_dtype_in_suffix(self):
        ...
        cfg_bf16 = _make_config("torch.bfloat16", is_mla_model=False, tp_size=4, model_name="Qwen2.5-7B")
        cfg_fp8 = _make_config("torch.float8_e5m2", is_mla_model=False, tp_size=4, model_name="Qwen2.5-7B")
        with tempfile.TemporaryDirectory() as tmpdir:
    ...
    def test_dtype_suffix_order_after_cp(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert dtype_pos > cp_pos, f"dtype suffix should come after CP suffix: {cache.config_suffix}"
...
class TestHf3fsDtypeKeyRoundTrip:
    ...
    def test_none_dtype_round_trips(self):
        ...
        """
        with tempfile.TemporaryDirectory() as tmpdir:
            backend = self._backend(os.path.join(tmpdir, "kv.bin"), self._metadata_client(), None)
            try:
                keys = ["page_x"]
