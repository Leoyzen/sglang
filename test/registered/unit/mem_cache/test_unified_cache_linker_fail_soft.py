...
class _RevalidatingFakeLinker(UnifiedCacheLinker):
    ...
    def pop_completed_offload(self) -> bool:
        return self.completed_offloads.pop(0) if self.completed_offloads else False
...
class _LoadBackComponent:
    ...
    def __init__(self):
        self.phases = []
        self.component_type = ComponentType.FULL
...
def _cache_for_load_back(component, empty_indices):
    ...
    cache = SimpleNamespace(
        ...
        resolve_node_handle=lambda node_id: SimpleNamespace(
            id=node_id, parent=SimpleNamespace(id=5, parent=None)
        ),
