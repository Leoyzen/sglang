...
class TestMambaDevicePoolGroup(CustomTestCase):
    def test_build_group_shape_and_order(self):
        group = _build_mamba_device_pool_group(
            _fake_hybrid_kvcache(), page_size=1, params=_fake_params()
        )
    ...
    def test_key_pointer_parity_per_layer(self):
        ...
        self.assertEqual(
            component_keys,
            ["slot-key_r0_tp0_temporal", "slot-key_r0_tp0_conv_0"],
        )
        ...
        ptrs, sizes, offsets = mamba.get_prepared_layer_range_meta(
            mamba.prepare_locations(torch.tensor([3])), 1
        )
        ...
        ptrs, _, _ = mamba.get_prepared_layer_range_meta(
            mamba.prepare_locations(torch.tensor([3])), 3
        )
    ...
    def test_offload_pack_uniform_groups(self):
        ...
        component_keys = [
            f"k{row}_{suffix}" for row in range(2) for suffix in ("temporal", "conv_0")
        ]
        packed_ptrs, packed_sizes = MooncakeStore._pack_multi_buffer_meta(
...
class TestMambaComponentTransfers(CustomTestCase):
        ...
        node = SimpleNamespace(
            ...
            component_data={
                ComponentType.MAMBA: SimpleNamespace(
                    value=torch.tensor([5], dtype=torch.int64)
                )
            },
        )
    ...
    def test_load_allocates_with_eviction_fallback(self):
        ...
        component.cache = SimpleNamespace(
            req_to_token_pool=SimpleNamespace(mamba_allocator=allocator),
            evict_for_alloc=lambda params: (
                evictions.append(params) or setattr(allocator, "next_slot", 0)
            ),
        )
        transfer = component.build_external_linker_transfer(
            LinkerTransferPhase.LOAD, None, ["h0", "h1"]
        )
...
class TestUpdateLoadCommitKeepsMamba(CustomTestCase):
    def test_commit_keeps_mamba_whole(self):
        ...
        wrapper = UnifiedCacheLinkerWrapperShim(cache)
    ...
    def test_resolve_transfers_keeps_mamba_trailing_policy(self):
        ...
        self.assertEqual(
            by_name[PoolName.MAMBA].hit_policy, PoolHitPolicy.TRAILING_PAGES
        )
...
class TestSelfKeyedBatchExists(CustomTestCase):
    ...
    def _store_stub(self, mamba_entry):
        ...
        def fake_batch_exist(keys):
            ...
            # exists iff its boundary key names the hit boundary
            # ("boundary_hit").
            return [
                1
                if (k.startswith("boundary_hit_") or not k.startswith("boundary_"))
                else 0
                for k in keys
            ]
...
class UnifiedCacheLinkerWrapperShim:
    """Minimal stand-in exposing _update_load without __init__."""
